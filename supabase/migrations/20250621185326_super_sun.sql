/*
  ...
  1. Novas Tabelas
    ...
    - `class_plans` - Planos de aula disponíveis
...
-- Políticas para leitura pública - VERIFICAR SE JÁ EXISTEM
DO $$ 
BEGIN
  -- Check and create policies for blog_posts
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'blog_posts' AND policyname = 'Allow public read access on blog_posts'
  ) THEN
  ...
  -- Check and create policies for class_settings
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'class_settings' AND policyname = 'Allow public read access on class_settings'
  ) THEN
  ...
  -- Check and create policies for class_plans
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'class_plans' AND policyname = 'Allow public read access on class_plans'
  ) THEN
...
-- Políticas para administração autenticada - VERIFICAR SE JÁ EXISTEM
DO $$ 
BEGIN
  -- Check and create policies for blog_posts
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'blog_posts' AND policyname = 'Allow authenticated users to manage blog_posts'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'class_settings' AND policyname = 'Allow authenticated users to manage class_settings'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'class_plans' AND policyname = 'Allow authenticated users to manage class_plans'
  ) THEN
...
DO $$
BEGIN
  -- Check and create trigger for blog_posts
  IF NOT EXISTS (
  ...
  -- Check and create trigger for class_settings
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'update_class_settings_updated_at'
  ) THEN
  ...
  -- Check and create trigger for class_plans
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'update_class_plans_updated_at'
  ) THEN
...
EXCEPTION
  WHEN undefined_column THEN
    -- This can happen if pg_trigger doesn't have tgname column in this version
    -- Just try to create the triggers and catch any errors
    BEGIN
      CREATE TRIGGER update_blog_posts_updated_at
        BEFORE UPDATE ON blog_posts
        FOR EACH ROW
...
VALUES
  ...
  (
    ...
    'inteligencia-artificial-desenvolvedores',
    '<h2>Inteligência Artificial para Desenvolvedores</h2><p>A inteligência artificial está transformando o desenvolvimento de software. Neste artigo, exploramos como desenvolvedores podem aproveitar ferramentas de IA para aumentar sua produtividade.</p>',
    'Como desenvolvedores podem aproveitar ferramentas de IA para aumentar sua produtividade.',
